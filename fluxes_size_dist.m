function flx0 = fluxes_size_dist(t,input0,dt,nbins,nspec,l_heat,t_res_heat,...
    T_f,T_i,ntot0,n_dist0,pstar,dHvap,T_ref,MW,sigma,rho,Dn,mu,press,alpha_m,alpha_t)
...
D(1:nspec) = Dn(1:nspec).*(T_TD./T_ref).^mu(1:nspec);
...
for j = 1:nbins+1
    ...
    sigmal(j) = sum(Xm(j,1:nspec).*sigma(1:nspec)); 
